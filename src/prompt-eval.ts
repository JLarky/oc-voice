...
const messages: TestMessage[] = singleMessage
  ? [
      {
        id: "adhoc",
        text: singleMessage,
        action: /\b(should|how|refactor|fix|could you|please)/i.test(
          singleMessage,
        ),
      },
    ]
  : [
      {
        id: "m1",
        text: "hey I just created the file what should I do with it",
        action: true,
      },
      {
        id: "m2",
        text: "Updated the dependency list; no action needed.",
        action: false,
      },
      {
        id: "m3",
        text: "Please refactor the function to improve performance.",
        action: true,
      },
      ...
      {
        id: "m7",
        text: "Need guidance on optimizing memory usage.",
        action: true,
      },
      {
        id: "m8",
        text: "Build succeeded after adjusting configs.",
        action: false,
      },
    ];
...
function parsePlainIntent(raw: string): boolean | null {
  ...
  if (/needs help|need guidance|should|how to|refactor|deploy/i.test(raw))
    return true;
  return null;
}
...
const prompts: PromptVariant[] = [
  {
    id: "p1_format_pipe",
    prompt:
      "Summarize the previous user message in <=12 words, then append |action=yes if they request help or guidance else |action=no. Output ONLY that line.",
    parse: parsePipe,
  },
  {
    id: "p2_classify_prefix",
    prompt:
      "Classify if the previous user message requests action/help: ACTION_REQUEST or INFORMATIONAL; then a 10-word summary. Format: <CLASS> <summary>.",
    parse: parseClassPrefix,
  },
  {
    id: "p3_intent_json",
    prompt:
      'Return JSON: {"summary":"<8 word summary>","action_request":true|false} analyzing ONLY the immediately preceding user message.',
    parse: parseJson,
  },
  {
    id: "p4_plain_intent",
    prompt:
      'Summarize the previous user message AND indicate if user asks for guidance. Example answer pattern: "agent needs help with file it just created" or "no help requested - dependency update noted". Output one concise sentence.',
  ...
  for (const msg of messages) {
    for (const pv of prompts) {
      const sessionId = await createSession(
        BASE,
        `prompt-eval-${msg.id}-${pv.id}`,
      );
      if (!sessionId) {
        console.warn("Session create failed", msg.id, pv.id);
        continue;
      }
      const send1 = await sendMessage(BASE, sessionId, msg.text);
      if (!send1.ok) {
        console.warn("Message send failed", msg.id, pv.id, send1.error);
        continue;
      }
      const send2 = await sendMessage(BASE, sessionId, pv.prompt);
      if (!send2.ok) {
        console.warn("Prompt send failed", msg.id, pv.id, send2.error);
        continue;
      }
...
function summarize(rows: EvalRow[]): void {
  ...
  for (const m of metrics) {
    console.log(
      `${m.promptId}\ttp:${m.tp}\tfp:${m.fp}\tfn:${m.fn}\ttn:${m.tn}\tunk:${m.unk}\tP:${m.precision.toFixed(2)}\tR:${m.recall.toFixed(2)}\tF1:${m.f1.toFixed(2)}`,
    );
  }
  console.log("\n=== ROWS ===");
  for (const r of rows) {
    console.log(
      `${r.promptId}\t${r.messageId}\tgold:${r.gold}\tpred:${r.pred}\t${r.raw}`,
    );
  }
}
