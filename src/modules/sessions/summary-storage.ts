...
const fsStorage = createStorage({ driver: fsDriver({ base: 'storage/summaries' }) });
const PREFIX = 'summary:';

export async function loadPersistedSummary(key: string): Promise<PersistedSummary | null> {
  try {
    // Try memory first
...
export async function persistSummary(key: string, data: PersistedSummary): Promise<void> {
  try {
...
export async function pruneExpired(ttlSuccessMs: number, ttlFailMs: number): Promise<void> {
  try {
    ...
    for (const full of keys) {
      ...
      const ttl = parsed.summary === '(summary failed)' ? ttlFailMs : ttlSuccessMs;
...
export async function appendSummaryLog(session: string, data: PersistedSummary): Promise<void> {
  ...
  try {
    ...
    // Add new entry
    filtered.push({ session, summary: data.summary, action: data.action, hash: data.hash, ts: data.ts });
    // Sort newest first
...
export async function getRecentSummaryLog(): Promise<SummaryLogEntry[]> {
  const entries = await loadLogRaw();
  const now = Date.now();
  return entries.filter((e) => now - e.ts < MAX_AGE_MS).sort((a, b) => b.ts - a.ts).slice(0, MAX_LOG_ENTRIES);
}
...
setInterval(() => {
  ...
  loadLogRaw().then((entries) => {
    ...
    if (capped.length !== entries.length || (entries[0] && capped[0] && entries[0].ts !== capped[0].ts)) {
      saveLog(capped);
    }
  }).catch(() => {});
}, 30000);
