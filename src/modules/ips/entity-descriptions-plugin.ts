...
import { listEntityIds, getEntityDescription, setEntityDescription } from '../../utils/store-entity-descriptions';
...
export const entityDescriptionsPlugin = new Elysia({ name: 'entity-descriptions' })
  ...
  .get('/entity-descriptions', async () => {
    try {
      ...
      console.error('entity-descriptions list route error', (e as Error).message);
      return { ok: false, entries: [] };
    }
  })
  // Fetch single description
  .get('/entity-descriptions/:id', ({ params }) => {
    const id = params.id;
    return Promise.resolve(getEntityDescription(id)).then((desc) => ({ ok: true, id, description: desc }));
  }, {
    params: t.Object({ id: t.String() })
  })
  // Create or update description via form body
  .post('/entity-descriptions/:id', async ({ params, body }) => {
    const id = params.id;
    let description = '';
    try {
      if (body && typeof body === 'object') {
        ...
        const cand = (body as any).description || (body as any).Description || (body as any).desc;
        if (typeof cand === 'string') description = cand;
      }
    } catch {}
    if (!description.trim()) return { ok: false, error: 'Missing description' };
    const ok = await setEntityDescription(id, description);
    return ok ? { ok: true, id, description } : { ok: false, error: 'Persist failed' };
  }, {
    params: t.Object({ id: t.String() }),
    body: t.Optional(t.Object({ description: t.Optional(t.String()), Description: t.Optional(t.String()), desc: t.Optional(t.String()) }))
  });
