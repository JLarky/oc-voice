...
import { listEntityIds, removeEntityDescription } from './utils/store-entity-descriptions';
...
describe('entity descriptions plugin', () => {
  test('lists empty initially', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/entity-descriptions'));
  ...
  test('sets and lists one description', async () => {
    ...
    const postRes = await app.handle(new Request('http://localhost/entity-descriptions/abc', { method: 'POST', body: fd }));
    const postBody = await json(postRes);
    expect(postBody.ok).toBe(true);
    expect(postBody.id).toBe('abc');
    expect(postBody.description).toBe('Test entity');

    const listRes = await app.handle(new Request('http://localhost/entity-descriptions'));
