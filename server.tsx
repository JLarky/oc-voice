...
import { MessagesList, AdvancedSdkJson, AdvancedInfo, StatusDiv, AdvancedEvents } from './rendering/fragments';
    ...
    async start(controller) {
      async function push() {
        try {
          ...
          // Update lightweight message count cache for reuse by advanced SSE
          messageCountCache[cacheKey] = { count: totalCount, updatedAt: Date.now() };
          // Periodic prune
...
const server = Bun.serve({
  ...
  async fetch(req: Request) {
    ...
    if (
      ...
      if (parts.length === 6 && parts[3] === 'advanced' && parts[4] === 'events' && parts[5] === 'stream') {
        const ip = parts[1];
        const sid = parts[2];
        ...
        // Candidate upstream SSE endpoints to probe relative to base
         ...
         const aggregatedState: any = {
           meta: { ip, sessionId: sid },
         ...
         function applyEventToState(obj: any) {
           ...
           if (typeof type === 'string' && type) {
             ...
             if (aggregatedState.lastTypes.length > 20) aggregatedState.lastTypes.splice(0, aggregatedState.lastTypes.length - 20);
           }
           // Message part updates
           if (obj?.data && typeof obj.data === 'object' && !Array.isArray(obj.data)) {
             try {
               if (type === 'message.part.updated') {
                 const part = (obj.data as any).part || (obj.data as any).data?.part;
                 if (part && typeof part === 'object') {
                   const pid = part.id || part.part_id || part.partId;
                   if (pid) {
                     aggregatedState.parts[pid] = {
                       type: part.type,
                       text: typeof part.text === 'string' ? part.text.substring(0, 200) : '',
                       updatedAt: Date.now()
                     };
                   }
                 }
               ...
               if (type === 'session.message.update') {
                 ...
                 if (typeof obj.data.count === 'number') aggregatedState.messageCount = obj.data.count;
                 if (typeof obj.data.role === 'string') aggregatedState.lastMessage.role = obj.data.role;
                 if (typeof obj.data.text === 'string') aggregatedState.lastMessage.text = obj.data.text.substring(0, 300);
               }
             } catch {}
           }
         }
         function serializeAggregatedState() {
           try {
             ...
             return json.length > MAX_STATE_JSON_LEN ? json.substring(0, MAX_STATE_JSON_LEN) + '...' : json;
           } catch { return ''; }
         }
        ...
        function enqueuePatch(controller: ReadableStreamDefaultController) {
           try {
             ...
             const jsx = <AdvancedEvents events={eventBuffer} attempts={attempts} stateJson={stateJson} />;
             controller.enqueue(new TextEncoder().encode(dataStarPatchElementsString(jsx)));
             lastPatchTs = Date.now();
             pendingPatch = false;
           } catch (e) {
             console.error('Advanced events enqueue error', (e as Error).message);
           }
         }
         ...
         async function openFirst(controller: ReadableStreamDefaultController) {
           ...
           async function tryNext() {
             ...
             try {
               ...
               const res = await fetch(urlStr, { signal: upstreamController.signal });
               ...
               if (!reader) { attempt.error = 'no body reader'; return tryNext(); }
               (async () => {
                 ...
                 try {
                   while (true) {
                     ...
                     while ((idx = buffered.indexOf('\n\n')) !== -1) {
                       ...
                       if (typeof obj.data === 'string' && obj.data.length > MAX_FIELD_LEN) obj.data = obj.data.substring(0, MAX_FIELD_LEN) + '...';
                       if (typeof obj.raw === 'string' && obj.raw.length > MAX_FIELD_LEN) obj.raw = obj.raw.substring(0, MAX_FIELD_LEN) + '...';
                        ...
                        if (!pendingPatch) {
                          pendingPatch = true;
                          const now = Date.now();
                          const dueIn = Math.max(0, PATCH_INTERVAL_MS - (now - lastPatchTs));
                          setTimeout(() => enqueuePatch(controller), dueIn);
                        }
                     }
                   }
                 } catch (e) {
                   console.error('Upstream SSE read error', (e as Error).message);
                 } finally {
                   upstreamClosed = true;
                   attempts.push({ url: urlStr, closed: true, events: eventCount, durationMs: Date.now() - startTs });
          ...
          async start(controller) {
              ...
              async function pollMessages() {
                try {
                  ...
                  if (count !== lastMessageCount) {
                    // Only append when count changes (new or removed messages).
                    const latest = msgs[msgs.length - 1];
                    const payload: any = {
                      type: 'session.message.update',
                      count,
                      role: latest?.role,
                      text: latest?.texts ? latest.texts.slice(-1)[0] : latest?.text || '',
                    };
                    const synthetic = { raw: 'data: ' + JSON.stringify(payload), data: payload };
                  ...
                  attempts.push({ notice: 'message poll error', error: (e as Error).message });
             ...
             setTimeout(() => {
               if (eventBuffer.length === 0) {
                 attempts.push({ notice: 'no events received yet', sinceMs: Date.now() - startTs });
                 enqueuePatch(controller);
               }
             }, 5000);
          ...
          async start(controller) {
            async function push() {
              try {
                ...
                const fresh = cachedCount && Date.now() - cachedCount.updatedAt <= MESSAGE_COUNT_FRESH_MS;
                if (fresh) {
                  ...
                  messageCountCache[cacheKey] = { count: approxCount, updatedAt: Date.now() };
                }
