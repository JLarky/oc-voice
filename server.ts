    ...
    if (url.pathname.startsWith("/sessions/")) {
      ...
      if (
        ...
        const page = `<!doctype html><html lang="en"><head><meta charset="UTF-8"/><title>Sessions for ${escapeHtml(
          ...
          ip
        )}</h1><div><a href="/">&larr; Back home</a></div><h2>Sessions</h2><button style="background:#e74c3c;color:#fff;margin-bottom:.5rem;padding:.25rem .5rem;font-size:.7rem;border:none;border-radius:3px;cursor:pointer" data-on:click="@post('/sessions/${escapeHtml(
          ip
        )}/clear-sessions')">Clear All</button><div id="sessions-status" class="status">Connecting...</div><div id="sessions-list" data-init="@get('/sessions/${escapeHtml(
          ip
